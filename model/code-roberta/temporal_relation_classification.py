...
from transformers import RobertaPreTrainedModel, RobertaModel, RobertaForSequenceClassification
...
class TemporalRelationClassification(RobertaForSequenceClassification):
    ...
    def __init__(self, config):
        ...
        self.bert: RobertaModel = RobertaModel.from_pretrained(config.base_lm)
    ...
    def _get_entities_and_start_markers_indices(self, input_ids):
        EMS1 = self.config.EMS1
        EMS2 = self.config.EMS2
        try:
            em1_s = [(ids == EMS1).nonzero(as_tuple=True)[0][0] if (ids == EMS1).nonzero(as_tuple=True)[0].numel() > 0 else None for ids in input_ids]
            em1_e = [(ids == EMS1).nonzero(as_tuple=True)[0][0] if (ids == EMS1).nonzero(as_tuple=True)[0].numel() > 0 else None for ids in input_ids]
            
            em2_s = [(ids == EMS2).nonzero(as_tuple=True)[0][0] if (ids == EMS2).nonzero(as_tuple=True)[0].numel() > 0 else None for ids in input_ids]
            em2_e = [(ids == EMS2).nonzero(as_tuple=True)[0][0] if (ids == EMS2).nonzero(as_tuple=True)[0].numel() > 0 else None for ids in input_ids]
        
        except IndexError:
            em1_s, em1_e, em2_s, em2_e = None, None, None, None
    ...
    def _max_pool_entity(self, mark_start, mark_end, sequence_output):
        ...
        for i, sentence in enumerate(sequence_output):
            if mark_start[i] < mark_end[i]:  # Ensure there is something to pool
                pooled, _ = torch.max(sentence[mark_start[i] + 1: mark_end[i]], dim=0, keepdim=True)
                pooled_entities.append(pooled)
            else:
            # Append zeros or handle appropriately if no valid range is found
                pooled_entities.append(torch.zeros_like(sentence[0:1]))  # Maintain the expected tensor shape

        return torch.stack(pooled_entities).reshape(sequence_output.shape[0], -1).to(self.device)
